...
export class HtmlEditor implements Component {
	...
	constructor(
		...
		this.editor = new Editor(null, (html) => htmlSanitizer.sanitizeFragment(html, {blockExternalContent: false}).html)
	...
	getValue(): string {
		if (this.mode === HtmlEditorMode.WYSIWYG) {
			...
			if (this.domTextArea) {
				return htmlSanitizer.sanitizeHTML(this.domTextArea.value, {blockExternalContent: false}).text
			} else {
				return this.value()
			}
		}
