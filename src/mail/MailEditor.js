...
export class MailEditor {
	...
	constructor(mailboxDetails: MailboxDetail) {
		...
		let supportButtonAtrrs = {
			label: () => "Support Feature",
			click: (e) => this._openTemplateFeature(this._editor.getCursorPosition()),
			icon: () => BootIcons.Help,
			noBubble: true
		}
		...
		this.subject._injectionsRight = () => {
			return this._allRecipients().find(r => r.type === RecipientInfoType.EXTERNAL)
				? [m(ButtonN, confidentialButtonAttrs), m(ButtonN, supportButtonAtrrs), m(ButtonN, attachFilesButtonAttrs), toolbarButton()]
				: [m(ButtonN, supportButtonAtrrs), m(ButtonN, attachFilesButtonAttrs), toolbarButton()]
		}
		...
		this.view = () => {
			return m("#mail-editor.full-height.text.touch-callout", {
				...
				m(".pt-s.text.scroll-x.break-word-links", {
					onclick: () => this._editor.focus()}, m(this._editor)),
				m(".pb")
			])
		}
		...
		this.dialog = Dialog.largeDialog(headerBarAttrs, this)
		                    ...
		                    .addShortcut({
			                    key: Keys.SPACE,
			                    shift: true,
			                    exec: () => this._openTemplateFeature(this._editor.getCursorPosition()),
			                    help: "send_action" // TODO: Add TranslationKey
		                    }).setCloseHandler(() => closeButtonAttrs.click(newMouseEvent(), this._domCloseButton))
		this._mailChanged = false
	}

	_openTemplateFeature(rect: ClientRect) {
		const onsubmit = (text) => {
			this._editor.insertHTML(text)
