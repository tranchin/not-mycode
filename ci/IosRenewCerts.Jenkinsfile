			...
			steps {
				...
				withCredentials([
					...
					dir('app-ios') {
						...
						// that we explicitly specify. Some of the env variables we use explicitly in app-ios/Fastfile,
						// some are used implicitly by fastlane.
						sh "GIT_SSH_COMMAND=\"ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no\" fastlane renewadhoccerttest"
						sh "GIT_SSH_COMMAND=\"ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no\" fastlane renewappstorecerttest"
					}
				}
			}
		}
