...
import {Scheduler} from "../../../../../src/api/common/utils/Scheduler.js"
...
o.spec("Indexer test", () => {
	...
	o.beforeEach(function () {
		schedulerMock = object<Scheduler>()
	})
	...
	o("_loadGroupDiff", function (done) {
		...
		let indexer = new Indexer(restClientMock, null as any, browserDataStub, entityRestCache, schedulerMock)
		indexer.db.dbFacade = {
			createTransaction: () => Promise.resolve(transaction),
		} as any
	...
	o("_updateGroups disable MailIndexing in case of a deleted mail group", async function () {
		let indexer = mock(new Indexer(restClientMock, null as any, browserDataStub, entityRestCache, schedulerMock), mock => {
	...
	o("_processEntityEvents_1", async function () {
		...
		const indexer = mock(new Indexer(restClientMock, null as any, browserDataStub, entityRestCache, schedulerMock), indexerMock => {
	...
	o("processEntityEvents non indexed group", function (done) {
		...
		const indexer = mock(new Indexer(restClientMock, null as any, browserDataStub, entityRestCache, schedulerMock), mock => {
			mock.db.initialized = Promise.resolve()
			mock._mail = {
				processEntityEvents: o.spy(() => Promise.resolve()),
			}
